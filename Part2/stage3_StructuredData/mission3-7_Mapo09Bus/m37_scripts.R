...
setwd('D:/Workspace/R_Data_Analysis/Part2/stage3_StructuredData/mission3-7_Mapo09Bus')
library(ggplot2)
...
bus09
length(bus09$승차인원) #32개 정류장 
bus09$number <- c(1:length(bus09$승차인원)) #컬럼추가됨 
bus09
stop <- bus09$정류소명
stop
...
ggplot(bus09, aes(x=정류소명)) +
  ...
  scale_x_continuous(breaks = 1:32, labels = stop) + #stop=정류소명
...
bus09
bus09$정류소명 <- factor(bus09$정류소명, levels=unique(bus09$정류소명))#x축 #합쳐지는게 있는 지 확인
bus09
